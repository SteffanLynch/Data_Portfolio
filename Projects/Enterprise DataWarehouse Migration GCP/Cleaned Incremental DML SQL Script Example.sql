...
(
 ...
 ,product_storage_temp as
  (
    select
    s.KEY_HASH_COLUMN as mergeKey,
    s.Product_Storage_UUID,
    s.SKU_ID,
    s.ROTATION_FLAG,
    s.STORAGE_SEGREGATION,
    s.LOCATION,
    s.PICKFACE,
    s.PICKFACE_CONTAINER,
    s.QUANTITY_PER_PALLET,
    s.QUANTITY_PER_TRAY,
    s.QUANTITY_PER_TOTE,
    s.QUANTITY_PER_BOX,
    s.STORAGE_SCHEME,
    ...
    S.KEY_HASH_COLUMN,
    s.NON_KEY_HASH_COLUMN,
    s.Record_Start_DateTime,
    --s.Record_End_DateTime,
    s.Is_Deleted,
    s.Inserted_By,
    s.Insert_DateTime,
    s.Modified_By,
    s.Modified_DateTime,
    s.Source_Name
  from Product_Storage as s
  WHERE
      NOT EXISTS ( --exlcude any records that match with the target table and is the latest
      SELECT
        1
      FROM
        `edw_product_storage` as T
      WHERE
        S.Key_Hash_Column = T.Key_Hash_Column
        AND s.Non_Key_Hash_Column = T.Non_Key_Hash_Column
        --AND s.EndTime = T.Record_End_DateTime
        )
 /* UNION ALL
  SELECT
    T.Key_Hash_Column as mergeKey,
    T.Product_Storage_UUID,
    T.SKU_ID,
    T.ROTATION_FLAG,
    T.STORAGE_SEGREGATION,
    T.LOCATION,
    T.PICKFACE,
    T.PICKFACE_CONTAINER,
    T.QUANTITY_PER_PALLET,
    T.QUANTITY_PER_TRAY,
    T.QUANTITY_PER_TOTE,
    T.QUANTITY_PER_BOX,
    T.STORAGE_SCHEME,
    ...
    T.KEY_HASH_COLUMN,
    T.NON_KEY_HASH_COLUMN,
    T.Record_Start_DateTime,
    --T.Record_End_DateTime,
    T.Is_Deleted,
    T.Inserted_By,
    T.Insert_DateTime,
    T.Modified_By,
  ...
  FROM
    `edw_product_storage` as T
    WHERE
      T.Record_End_DateTime = TIMESTAMP('2999-12-31 23:59:59+00')
      AND EXISTS (
      SELECT
      1
      FROM
      product_storage as S
      WHERE
      S.Key_Hash_Column = T.Key_Hash_Column
      --AND s.Record_Start_DateTime = t.Record_Start_DateTime
      AND (S.Non_Key_Hash_Column <> T.Non_Key_Hash_Column)
  ...
  select
    ...
    CASE when date(endtime) <> '2999-12-31' then endtime
      WHEN (LEAD(product_storage_temp.Key_Hash_Column) 
    OVER (PARTITION BY product_storage_temp.Key_Hash_Column 
    ORDER BY product_storage_temp.Record_Start_DateTime)) 
    IS NULL THEN TIMESTAMP('2999-12-31 23:59:59+00')
    ELSE
    CURRENT_TIMESTAMP()
  END
   AS Record_End_DateTime
  FROM
   product_storage_temp
   LEFT OUTER JOIN PRODUCT_STORAGE
    ON 
     PRODUCT_STORAGE_TEMP.SKU_ID = PRODUCT_STORAGE.SKU_ID 
     AND PRODUCT_STORAGE_TEMP.Record_Start_Datetime = PRODUCT_STORAGE.Record_Start_DateTime
) as src  
ON
  src.mergeKey=tgt.Key_Hash_Column and
  src.record_start_datetime = tgt.record_start_datetime
 -- and src.Non_Key_Hash_Column = tgt.Non_Key_Hash_Column 
    
WHEN MATCHED 
THEN
  UPDATE SET
    tgt.Record_End_DateTime = src.Record_End_DateTime, -- used to be SRC.Record_End_DateTime, i think this is where the update issue was.
    Modified_By = 'dataplatform-edw',
...
THEN
  ...
  VALUES
  (
  src.Product_Storage_UUID,
    src.SKU_ID,
    src.ROTATION_FLAG,
    src.STORAGE_SEGREGATION,
    src.LOCATION,
    src.PICKFACE,
  src.PICKFACE_CONTAINER,
  src.QUANTITY_PER_PALLET,
  src.QUANTITY_PER_TRAY,
  src.QUANTITY_PER_TOTE,
  src.QUANTITY_PER_BOX,
  src.STORAGE_SCHEME,
  src.STORAGE_TEMPERATURE,
  src.KEY_HASH_COLUMN,
  src.NON_KEY_HASH_COLUMN,
  src.Record_Start_DateTime,
  src.Record_End_DateTime,
  src.Is_Deleted,
    src.Inserted_By,
    src.Insert_DateTime,
    src.Modified_By,
